    ...
    return (
      ...
      >
        ...
        {helperText && (
          <FormHelperText
            {...FormHelperTextProps}
            className={FormHelperTextProps ? clsx(FormHelperTextProps.className, classes.helperText) : classes.helperText}
          >
            {helperText}
          </FormHelperText>
        )}
...
const WAMuiChipInput = withStyles(styles, { name: 'WAMuiChipInput' })(ChipInput)

const MyChipInput = (p) => {
  const isControlledMode = !!p.value

  let onAdd
  let onDelete
  if(isControlledMode){
    onAdd = v => p.onChange(p.value.concat(v))
    onDelete = v => p.onChange(p.value.filter(v_ => v !== v_))
  }
  else {
    onAdd = p.onAdd
    onDelete = p.onDelete
  ...
  return <WAMuiChipInput {...p} onAdd={onAdd} onDelete={onDelete} />
}

MyChipInput.propTypes = ChipInput.propTypes
