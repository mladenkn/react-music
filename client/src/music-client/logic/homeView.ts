...
const initialYoutubeTrackForm: YoutubeTrackQueryFormData = {
...
export const useHomeViewLogic = () => {
  ...
  const [state, setState] = useState({
    ...
    youtubeTrackForm: initialYoutubeTrackForm
  });
  ...
  useEffect(() => {
    if (state.youtubeTrackForm !== initialYoutubeTrackForm)
      fetchTracksFromYtDebounced();
  }, [state.youtubeTrackForm]);

  const setQuerySelection = (value: QueryTypeSelection) =>
    setState({ ...state, querySelection: value });

  const setResultSelection = (value: QueryResultSelection) =>
    setState({ ...state, resultSelection: value });

  const setyoutubeTrackForm = (f: YoutubeTrackQueryFormData) =>
    setState({ ...state, youtubeTrackForm: f });

  const trackDataFormChange = (input: TrackDataForm) => {
    setState({ ...state, trackDataFormState: input });
  };

  const [fetchTracksDebounced] = useDebouncedCallback(
    () => wrapped.fetchTracks(state.trackDataFormState),
    500
  );
  const [fetchTracksFromYtDebounced] = useDebouncedCallback(
  ...
  const tracksToDisplay =
    state.resultSelection === QueryResultSelection.QueryResult
      ? wrapped.tracks.data
      : wrapped.trackRecommendations;
  const displayedTracks = tracksToDisplay.map(
    mapTrack(wrapped.userPermissions)
  );
  const tracksTotalCount = wrapped.tracks.totalCount;

  const onSaveTrack = (
    trackEditableProps: TrackDataEditableProps & { id: string }
  ) => {
    const track_ = wrapped.allTracks.find(
      t => t.ytId === trackEditableProps.id
    )!;
  ...
  const [selectedItemId, setSelectedItemId] = useState<undefined | string>(
    undefined
  );
  ...
  const fetchRecommendationsOf = async (trackId: string) => {
    ...
    setState({
      ...state,
      resultSelection: QueryResultSelection.Recommedations
    });
  };
...
export const mapTrack = (permissions: UserPermissions) => (
  t: TrackData
): TrackViewData => {
  ...
  if (!rootProps.channel) console.log("nema channel", rootProps, t);
  if (!t.ytId) {
    debugger;
